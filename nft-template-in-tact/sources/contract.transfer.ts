...
(async () => {
  ...
  let latest_indexId = (await nftContract.getGetCollectionData()).nextItemIndex;
  console.log("Next indexID:[", latest_indexId, "]");
  const itemAddress = await nftContract.getGetNftAddressByIndex(latest_indexId - BigInt(1));
  console.log("NFT Item: ", itemAddress);

  const itemContract = client4.open(NftItemTemplate.fromAddress(itemAddress));
  console.log("Item NftData: ", await itemContract.getGetNftData());

  const addressTo = Address.parse("UQDWNbrOWiegJoQ7CqYiIwN5Kb1nRtFH_QXKTKAavomFvwYa")
  ...
  console.log("New Item Owner: ", (await itemContract.getGetNftData()).ownerAddress);
})();
